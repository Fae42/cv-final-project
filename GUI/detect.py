...
class Ui_MainWindow(object):
    ...
    def startAction(self):
        ...
        face_objs = DeepFace.extract_faces(imgNamepath, enforce_detection=False
                                           # (224, 224),
                                           # backends[4]
                                           )
        print('-------')
        ...
        for face in face_objs:
            ...
            top = region['y']  # 上边界位置
            left = region['x']  # 左边界位置
            width = region['w']  # 边界框宽度
            height = region['h']  # 边界框高度
            cv2.rectangle(result, (left, top), (left + width, top + height), (0, 255, 0), 5)
        # face = face_objs.get('face')
        result = cv2.cvtColor(result, cv2.COLOR_BGR2RGB)
        height, width, channels = result.shape
        bytes_per_line = channels * width
        img = QImage(result.data, width, height, bytes_per_line, QImage.Format_RGB888)
        pixmap = QtGui.QPixmap.fromImage(img).scaled(self.label_4.width(), self.label_4.height())
